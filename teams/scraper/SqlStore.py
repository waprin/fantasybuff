...
class SqlStore:
    ...
    def has_game(self, league, team_id, week):
        return GameHtmlScrape.objects.filter(Q(first_team=team_id) | Q(second_team=team_id), league=league,
                                             week=week).count() > 0

    def get_game(self, league, team_id, week):
        return GameHtmlScrape.objects.filter(Q(first_team=team_id) | Q(second_team=team_id), league=league, week=week)[
            0].html
